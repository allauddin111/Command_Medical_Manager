...
void medi_sale()
{
  ...
  y=now->tm_year;
	...
	char comp_name[20];
	char supp_name[30];
			   while(fscanf(ptr,"%s %s %d %s  %d %f%f%f%f%f%f%d%s %s %s %d%s %s\n",temp.id,temp.medi_name,&temp.rack,temp.cabnit,&temp.quantity,&temp.sale,&temp.total,&temp.unit,&temp.cost,&temp.profit,&temp.bye,&temp.qty,temp.pur_date,temp.exp_date,temp.manu_date,&temp.bill_no,temp.comp_name,temp.supp_name)!=EOF)
		{
			gotoxy(53,i);
			printf("sdfds%s",temp.id);
			gotoxy(64,i);
			printf("sdfsd%s",temp.medi_name);
			i+=2;
			char medi_id[6];
	char medir_name[20];
	char supp_name[30];
